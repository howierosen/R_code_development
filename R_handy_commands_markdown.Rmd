...
### Get marginal means from a regression model
      ...
      lsmeans(model_name, variable1, by=factor_variable)
...
library(nlme)
mlm_vis_vol1 <- lme (volume1 ~ visit * dx,  random =~ 1|PIDN, data=test.frame.3c, method = "REML")
anova(mlm_vis_vol1)
summary(mlm_vis_vol1)
library(car)
scatterplot(test.frame.3c$volume1~test.frame.3c$visit | test.frame.3c$dx)
