...
```{r load_show_3cat_test.frame}
test.frame.3c <- read.csv("test.frame.3c.csv")
test.frame.3c

#str(test.frame.3c) # note that visit is an integer, but it should be a factor, we will 
# test.frame.3c$visit <- as.ordered(test.frame.3c$visit)
# class(test.frame.3c$visit)
# levels(test.frame.3c$visit)
...
### Converting an integer variable to a factor (categorical)

      as.factor(variable_name)
      
e.g.

```{r convert_to_factor}

test.frame.3c.int2fac <- read.csv("test.frame.3c.csv")
class(test.frame.3c.int2fac$visit)
test.frame.3c.int2fac$visit <- as.factor(test.frame.3c.int2fac$visit)
class(test.frame.3c.int2fac$visit)
levels(test.frame.3c.int2fac$visit)

```

      
### Converting an integer variable to an ordinal factor

      as.ordered(variable_name)

```{r convert_to_ordered}

test.frame.3c.int2ord <- read.csv("test.frame.3c.csv")
class(test.frame.3c.int2ord$visit)
test.frame.3c.int2ord$visit <- as.ordered(test.frame.3c.int2ord$visit)
class(test.frame.3c.int2ord$visit)
levels(test.frame.3c.int2ord$visit)
...
### Dropping levels from a subset

NOTE that subsetting does not usually drop the unused levels from the new subset, it just doesn't have any data in them. This can mess up certain programs like cohensD. To completely drop teh levels, use the droplevels command

e.g.

``` {r subset_levels}

# here is the original data frame, which has 3 levels: AD, BV and CON
test.frame.3c
# exclude the BV cases and put the results in a new object
test.2c <- subset(test.frame.3c, test.frame.3c$dx=="CON" |  test.frame.3c$dx=="AD")
# new object shows only 2 levels in dx: AD and CON
test.2c
# but levels command shows the BV level is still in the new object
levels(test.2c$dx)
# droplevels commands clears the unused level
test.2c <- droplevels(test.2c)
levels(test.2c$dx)
...
### Run anova on a linear model you've already created
...
e.g.
Model below will test for effect of age on volume, including looking for possible interaction between age and diagnosis in predicting volume. This is using the data frame with 3 levels for diagnosis

```{r regress_vol_dx_by_age}

# here is the model with interaction included
lm_age_dx <- lm(test.frame.3c$volume1 ~ test.frame.3c$age*test.frame.3c$dx)
anova(lm_age_dx)
summary(lm_age_dx)

# the anova table indicates that there is no interaction between age and diagnosis, but there is a significant relationship between age and volume1. In th above model, the age coefficient is just in AD. Now we'll rerun the model without the interaction, so that we get the coefficient for age

lm_age_dx2 <- lm(test.frame.3c$volume1 ~ test.frame.3c$age + test.frame.3c$dx)
anova(lm_age_dx2)
summary(lm_age_dx2)
...
### Get marginal means from a regression model

      lsmeans(model_name, factor_variable1, by=other_factor_variable)
...
library(lsmeans)
lsmeans(lm_age_dx2, "test.frame.3c$dx")

```

### Rerun prior regression with different predictors	
...
This is syntax using lme, part of nlme package.

    Object_name <- lme (dep_variable ~ indep_variable1 + indep_variable2 + indep_varialble3 + ..., random =~ 1 | random_variable, data = dataset_name)

e.g.
below, we are looking to see whether there is a an effect of visit on volume1, and whether there is an interaction wiht diagnosis

``` {r mixed_visit_volume1}

# here is the model with interaction included. Shows there is an effect of visit and an interaction with diagnosis, such that BV has a bit less volume1 loss per visit compared with AD. CON also shrink less than AD, but they have more variability at each timepoint, so difference vs AD is not significant

test.frame.3c$visit <- ordered(test.frame.3c$visit)
class(test.frame.3c$visit)
library(nlme)
mlm_vis_vol1 <- lme (volume1 ~ visit +  dx, random =~ 1|PIDN, data=test.frame.3c)
anova(mlm_vis_vol1)
summary(mlm_vis_vol1)
library(car)
# bwplot(test.frame.3c$volume1~test.frame.3c$visit |  test.frame.3c$dx)
lsmeans(mlm_vis_vol1, "visit", by="dx") # note the marginal means don't look like the raw means shown in the boxplots, because they are adjusted for age

```

Can also run linear mixed model using lmer funciton, part of the lme4 package. This uses restricted maximum likelihood. Syntax is a bit different:

      Object_name <- lmer (dep_variable ~ indep_variable1 + indep_variable2 + indep_varialble3 + (1 | random_variable), data = dataset_name)

```{r mixed_visit_volume1_alt}

library(lme4)
mlm_vis_vol2 <- lmer(volume1 ~ visit * dx + (1|PIDN), data=test.frame.3c)
anova(mlm_vis_vol2)
summary(mlm_vis_vol2)
...
### Cohen's d with subsetting

Cohen's d will only work if there are two levels in the factor you're using to compare levels. If you have a dataset with more than 2 levels, you have to subset to make a new dataset with only two levels. For some reason, in order to use cohen's d, you have to apply drop levels command. For example, below we'll pick two levels to test in our dataset that has 3 levels

```{r drop_levels_for_cohensD}

co_d_test <- with(test.frame.3c, test.frame.3c[test.frame.3c$dx!="CON", ])
co_d_test <-  droplevels(co_d_test)
cohensD(co_d_test$age ~ co_d_test$dx)
